...
class LintRunner {
     ...
     * @returns A promise that resolves when linting is complete.
     */
    async lintWorkspace(filePaths, config, progress, token) {
     ...
     */
    async runPCLintPlus(files, config) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (const configFile of configFiles) {
                ...
                if (configFile.includes('${workspaceFolder}')) {
                    resolvedPath = configFile.replace('${workspaceFolder}', vscode.workspace.workspaceFolders
                ...
                const defaultLocation = path.join(config.repositoryPath || process.cwd(), 'cfg', 'pclint_plus', path.basename(configFile));
            ...
            for (const includePath of includePaths) {
                ...
                const resolvedPath = cleanPath.replace('${workspaceFolder}', vscode.workspace.workspaceFolders
                    ? vscode.workspace.workspaceFolders[0].uri.fsPath
                    : process.cwd());
            ...
            if (config.useVSCodeIncludes) {
                ...
                try {
                    ...
                    const existingPaths = new Set(includePaths.map(p => path.normalize(p.replace('${workspaceFolder}', vscode.workspace.workspaceFolders
                        ? vscode.workspace.workspaceFolders[0].uri.fsPath
                        : process.cwd()))));
                    let vsCodePathsAdded = 0;
                    for (const includePath of vsCodeIncludes) {
                        // Resolve any workspace folder variables
                        const resolvedPath = includePath.replace('${workspaceFolder}', vscode.workspace.workspaceFolders
                            ? vscode.workspace.workspaceFolders[0].uri.fsPath
                            : process.cwd());
     ...
     */
    parseLintOutput(output, sourceFilePath, clearExistingDiagnostics = true) {
        if (!output || output.trim() === '') {
        ...
        if (output.includes('<lintanalysis>')) {
            ...
            if (sourceFileIssues === 0 && (headerFileIssues > 0 || otherFileIssues > 0)) {
                ...
                const summaryDiagnostic = new vscode.Diagnostic(new vscode.Range(0, 0, 0, 50), summaryMessage, vscode.DiagnosticSeverity.Information);
     ...
     */
    async addVSCodeIncludePaths(config) {
     ...
     */
    async addVSCodeIncludePathsToConfig(config, tempConfigContent) {
        if (config.useVSCodeIncludes) {
            try {
                const vsCodeIncludes = await this.getVSCodeIncludePaths();
                this.outputChannel.appendLine(`Processing ${vsCodeIncludes.length} VSCode include paths`);
                ...
                const newIncludes = vsCodeIncludes.filter(vsPath => !existingPaths.some(existing => 
                // Compare normalized paths
                path.normalize(vsPath.replace('${workspaceFolder}', vscode.workspace.workspaceFolders
                    ? vscode.workspace.workspaceFolders[0].uri.fsPath
                    : process.cwd())) === path.normalize(existing.replace('${workspaceFolder}', vscode.workspace.workspaceFolders
                    ? vscode.workspace.workspaceFolders[0].uri.fsPath
                    : process.cwd()))));
