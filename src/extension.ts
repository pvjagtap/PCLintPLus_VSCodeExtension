...
import { LintConfig } from './lintRunner';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    lintRunner = new LintRunner(diagnosticCollection, outputChannel);
    
    // Register commands
    context.subscriptions.push(
        vscode.commands.registerCommand('vscode-pclint-plus.lintCurrentFile', () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('vscode-pclint-plus.lintWorkspace', () => {
            lintWorkspace();
        })
    );
    ...
    context.subscriptions.push(
        vscode.workspace.onDidSaveTextDocument(document => {
            ...
            if (config.get<boolean>('autoLintOnSave')) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('vscode-pclint-plus.clearDiagnostics', () => {
 ...
 */
async function lintCurrentFile(document: vscode.TextDocument) {
    ...
    vscode.window.setStatusBarMessage('PC-Lint Plus: Linting current file...', 
        lintRunner.lintFile(document.uri.fsPath, config)
            .then(() => {
...
async function lintWorkspace() {
    ...
    try {
        ...
        const filePatterns = vscode.workspace.getConfiguration('pclintplus').get<string[]>('filePatterns', ['**/*.c', '**/*.h']);
        const excludePatterns = vscode.workspace.getConfiguration('pclintplus').get<string[]>('excludePatterns', ['**/node_modules/**', '**/build/**', '**/out/**']);
        ...
        for (const dir of searchDirectories) {
            ...
            const findFiles = (directory: string, recursive: boolean = true): string[] => {
                const result: string[] = [];
 ...
 */
function getConfiguration(): LintConfig {
    const config = vscode.workspace.getConfiguration('pclintplus');
    const workspaceFolders = vscode.workspace.workspaceFolders;
    ...
    const includePaths = (config.get<string[]>('includePaths') || []).map(path => 
        path.replace('${workspaceFolder}', workspaceRoot)
    );
    
    // Add compiler include path if configured
    const compilerIncludePath = config.get<string>('compilerIncludePath', '');
    ...
    const userDefs = config.get<string[]>('preprocessorDefinitions', []);
    const preprocessorDefinitions = [...new Set([...userDefs, ...defaultPreprocessorDefs])];
    
    return {
        executablePath: config.get<string>('executablePath', ''),
        configFiles: config.get<string[]>('configFiles', []),
        repositoryPath: config.get<string>('repositoryPath', workspaceRoot),
        includePaths: includePaths,
        preprocessorDefinitions: preprocessorDefinitions,
        additionalArgs: config.get<string[]>('additionalArgs', ['-b', '-width(240,4)']),
        useVSCodeIncludes: config.get<boolean>('useVSCodeIncludes', true),
        outputFormat: config.get<string>('outputFormat', 'xml')  
    };
}
